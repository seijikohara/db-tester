...
区切り形式（CSV、TSV）は非nullの空文字列を表現できません。空のフィールドと空のクォートフィールドは
どちらもSQL NULLにマッピングされます:
...
空文字列とNULLを区別するには、JSONまたはYAMLを使用します。これらの形式は`null`と`""`に対して
異なる構文を提供します:
...
| 空文字列処理 | JSONの`""`はNULLとは異なる非nullの空文字列にマッピング |
...
| 空文字列処理 | YAMLの`""`はNULLとは異なる非nullの空文字列にマッピング |
| 値変換 | すべての非null値は文字列に変換 |
