...
データの不一致ではなく、検証を完了できなかったことを示します。

**一般的な原因**:

- `ComparisonMode.STRICT`下で、柔軟な比較戦略のために値をパースできない
- 比較戦略を値に適用できない
- リトライ待機中に検証が中断された

データの不一致（行数差やカラム値差）は`ValidationException`ではなく`java.lang.AssertionError`をスローします。アサーションAPIとアノテーション駆動の経路はどちらも不一致を`AssertionError`として表面化するため、単一の`catch (AssertionError)`で両方の入口を扱えます。

**出力形式**: 不一致の`AssertionError`は人間が読みやすい要約に続いてYAML詳細を持ちます。形式の詳細は[エラーハンドリング - 検証エラー](error-handling#検証エラー)を参照してください。
