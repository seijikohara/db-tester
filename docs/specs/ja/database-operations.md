...
**ユースケース**: 以前のテストからデータが存在するExpectedDataSetのみのテスト、または手動セットアップシナリオ。
...
**ユースケース**: 既存データを持つテーブルへの追加、または特定の行追加を必要とするテスト。

**制約**: ターゲット行が空であるか、一意のキーが必要。
...
**ユースケース**: 既存データが部分的に存在する混合シナリオ、増分データセットアップ、または冪等なテストデータ準備。
...
**ユースケース**: 特定のテストレコードの削除や個別行のクリーンアップ。

**要件**: データセットに主キーカラムが含まれていること。
...
**ユースケース**: シーケンスを保持しながらテーブルをクリア、または挿入専用操作のセットアップ。

**テーブル処理順序**: テーブルは外部キーの逆順で処理。
...
**ユースケース**: シーケンスを含む完全なテーブルリセット、またはパフォーマンス重視のクリーンアップ。
...
**ユースケース**: 標準的なテスト準備（デフォルト操作）。決定論的な初期状態の確立。
...
**ユースケース**: シーケンスリセット付きのテスト準備、またはパフォーマンス最適化されたセットアップ。
...
`TableOrderingStrategy`列挙型は、データベース操作中のテーブル処理順序の決定方法を制御します。
...
データセットディレクトリに`load-order.txt`ファイルが必要です。ファイルが存在しない場合、フレームワークは`DataSetLoadException`をスローします。
...
**ユースケース**: テーブル順序が重要でない場合（FK制約がない）、または依存関係のないシナリオでの決定論的な順序付け。
...
ファイル形式と使用方法の詳細については、[データフォーマット - 読み込み順序](data-formats#読み込み順序)を参照してください。

### 外部キーの認識

`load-order.txt`ファイルが存在しない場合、本フレームワークはデータベースメタデータを使用してテーブル依存関係を解決します。
...
**注意**: DB Testerは`load-order.txt`ファイルを自動生成しません。詳細は[テーブル順序戦略](#テーブル順序戦略)を参照してください。

### 循環依存

循環外部キー参照を持つテーブルの場合の処理:
...
本フレームワークは、SQL文に識別子を挿入する前にすべてのSQL識別子（テーブル名とカラム名）を検証します。不正な入力によるSQL構文エラーやセキュリティ問題を防止します。
...
本フレームワークは、データセットファイルの文字列値を対応するSQL型に変換します。
...
- [パブリックAPI](public-api) - Operation enumリファレンス
- [データフォーマット](data-formats) - ソースファイル構造
- [設定](configuration) - OperationDefaults
- [エラーハンドリング](error-handling) - データベース操作エラー
