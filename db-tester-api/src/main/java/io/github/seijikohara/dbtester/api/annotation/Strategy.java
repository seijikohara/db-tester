...
public enum Strategy {
   ...
   * requiring a pattern, use {@link #toComparisonStrategy(String)}.
   *
    ...
    return switch (this) {
      ...
      case REGEX ->
          throw new IllegalStateException(
              "REGEX strategy requires a pattern. " + "Use toComparisonStrategy(String) instead.");
    };
  }
   ...
   *
   * <p>The pattern parameter is used for {@link #REGEX} strategy. The options parameter is reserved
   * for future use.
   *
   * @param pattern the regex pattern for REGEX strategy, may be empty for non-REGEX strategies
   * @param options the strategy-specific options (reserved for future use), may be empty
   * @return the corresponding ComparisonStrategy instance
   * @throws IllegalArgumentException if REGEX strategy is used with an empty pattern
   */
